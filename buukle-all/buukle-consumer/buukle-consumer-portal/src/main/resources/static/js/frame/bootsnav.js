...
function renderArticleContainer(data) {
    for(var i = 0; i < data.length; i++){
        var artElement = "<div class='col-lg-4 col-md-3 col-sm-4'>" +
            "<a data-id='"+data[i].articleInfo.id+"' class='article-detail' href='articleInfo/detail/"+base.encode(data[i].articleInfo.id)+"' target='_blank'><img src='images/1.png' title='' alt=''></a>" +
            "<div class='art-info'>"+
            ...
            "</div>" +
            "<div class='art-stars'>" +
            "<i class='fa fa-eye'></i> <span class='eye'>&nbsp;"+data[i].articleInfo.bak01+"</span>" +
            "<i class='fa fa-heart'></i> <span class='star'>&nbsp;"+data[i].articleInfo.likeNumber+"</span> " +
...
/*渲染分页组件*/
function renderPagination(data,tempId) {
    $(function () {
        var totalPages = data.count/12 +(data.count%12>0 ? 1 :0);
        $("#page").bootstrapPaginator({
            bootstrapMajorVersion:3,            //对应的bootstrap版本
            currentPage: data.page,             //当前页数
            numberOfPages: 12,                   //每次显示页数
            totalPages:totalPages,              //总页数
            shouldShowPage:true,                //是否显示该按钮
            useBootstrapTooltip:true,
            //点击事件
            onPageClicked: function (event, originalEvent, type, page) {
                loadArticleContainer(page,tempId);
            }
        });
    });
}
