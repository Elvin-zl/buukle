...
import top.buukle.consumer.portal.constants.PortalPrivateKeyConstant;
...
public class SecuritySignInterceptor implements RequestInterceptor {
    ...
    public void apply(RequestTemplate requestTemplate) {
        if(requestTemplate.url().startsWith("/api/")){
            try {
                requestTemplate.header(SignUtil.SECURITY_SIGN_KEY,SignUtil.sign(new String(requestTemplate.body()), PortalPrivateKeyConstant.SECURITY_API_PRIVATE_KEY));
            } catch (Exception e) {
                e.printStackTrace();
                LOGGER.info("签名写入异常!");
            }
